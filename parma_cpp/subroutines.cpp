...
const double coshigh = 0.707106781186547; // 1/sqrt(2) (45 degree)
const double coslow = 0.50;               // 1/2 (60 degree)

if(ip==4 && g >= 0 && g <=1.0 && ang < coshigh) { // ground level muon
 emin=1.1535e4;
 if(e>=emin) {
 ...
 if(ang > coslow) { // Partial correction for angle between 45 and 60 2024/11/10
  ratio = (1.0 / ang - 1.0 / coshigh) / (1.0 / coslow - 1.0 / coshigh); // interpolated with 1 / cos
  getSpecAngFinal = getSpecAngFinal * ratio + getSpecAngCpp(ip, s, r, d, e, g, ang) * (1.0 - ratio);
 }
} else {
